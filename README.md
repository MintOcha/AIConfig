...
- `prompts/shared.md` contains the shared coding preferences and guardrails.
...
```text
@/absolute/path/to/AIConfig/prompts/shared.md
```
...
The shared `prompts/shared.md` is always available. Add more Markdown files under
...
Point the agent's global instruction file at `prompts/shared.md`, or copy its contents when the agent does not support imported instruction files. Skill support and installation locations vary by agent, so treat each directory under `skill/` as a self-contained package and follow that agent's skill-loading convention.
